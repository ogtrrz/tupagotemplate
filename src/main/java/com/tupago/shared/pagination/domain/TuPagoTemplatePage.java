...
import com.tupago.shared.collection.domain.TuPagoTemplateCollections;
import com.tupago.shared.error.domain.Assert;

public class TuPagoTemplatePage<T> {
  ...
  private TuPagoTemplatePage(TuPagoTemplatePageBuilder<T> builder) {
    content = TuPagoTemplateCollections.immutable(builder.content);
  ...
  public static <T> TuPagoTemplatePage<T> singlePage(List<T> content) {
    return builder(content).build();
  }

  public static <T> TuPagoTemplatePageBuilder<T> builder(List<T> content) {
    return new TuPagoTemplatePageBuilder<>(content);
  }

  public static <T> TuPagoTemplatePage<T> of(List<T> elements, TuPagoTemplatePageable pagination) {
  ...
  public <R> TuPagoTemplatePage<R> map(Function<T, R> mapper) {
  ...
  public static class TuPagoTemplatePageBuilder<T> {
    ...
    private TuPagoTemplatePageBuilder(List<T> content) {
      this.content = content;
    }

    public TuPagoTemplatePageBuilder<T> pageSize(int pageSize) {
      this.pageSize = pageSize;

      return this;
    }

    public TuPagoTemplatePageBuilder<T> currentPage(int currentPage) {
      this.currentPage = currentPage;

      return this;
    }

    public TuPagoTemplatePageBuilder<T> totalElementsCount(long totalElementsCount) {
      this.totalElementsCount = totalElementsCount;

      return this;
    }

    public TuPagoTemplatePage<T> build() {
      return new TuPagoTemplatePage<>(this);
    }
  }
}
